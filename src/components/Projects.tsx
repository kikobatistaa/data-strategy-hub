...
const Projects = () => {
  ...
  const projects = [
    { title: t.volkswagen.title, category: t.volkswagen.category, icon: Briefcase, tags: t.volkswagen.tags, isLive: true, isAcademic: true, academicBadge: "Strategy Consultant", grade: "Final Project", isCaseStudy: true, caseStudyData: t.volkswagen.caseStudy, buttons: t.volkswagen.buttons },
    { title: t.bank.title, category: t.bank.category, icon: LineChart, tags: t.bank.tags, isLive: true, isAcademic: true, academicBadge: t.bank.academicBadge, grade: t.bank.grade, isCaseStudy: true, caseStudyData: t.bank.caseStudy, buttons: t.bank.buttons },
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Horizontal scroll on desktop, stagger on mobile
      const isMobile = window.innerWidth < 1024;
      if (!isMobile && scrollContainerRef.current && sectionRef.current) {
        ...
        cards.forEach((card) => {
          gsap.fromTo(
            ...
            { opacity: 0.3, scale: 0.95 },
            {
              ...
              scrollTrigger: {
                trigger: card,
                containerAnimation: gsap.getById?.("projects-scroll") || undefined,
                start: "left 80%",
                end: "left 50%",
                scrub: true,
              },
  ...
  return (
    <section id="projects" ref={sectionRef} className="py-24 bg-secondary/30 overflow-hidden">
      ...
      <div
        ...
        className={
          isDesktop
            ? "flex gap-8 px-6 lg:px-12 w-max"
            : "container mx-auto px-6 grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl"
        }
      >
        {projects.map((project, index) => {
          ...
          return (
            <Card
              key={index}
              className={`project-card card-mouse-glow group border border-white/10 shadow-card hover:shadow-hover hover:-translate-y-2 transition-all duration-500 relative overflow-hidden bg-card/50 backdrop-blur-md hover:border-accent/50 rounded-2xl flex flex-col ${
                isDesktop ? "w-[380px] flex-shrink-0" : ""
              }`}
                    ...
                    <Dialog>
                      ...
                      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
                        ...
                        <div className="pt-4 mt-auto">
                          <Button className="w-full gap-2" onClick={() => window.open(pdfUrl, "_blank")}>
